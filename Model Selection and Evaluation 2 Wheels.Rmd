...
subtitle: "Model Selection and Evaluation - 2 Wheels Vehicles (Arrested)"
...
requirements <- c(
  ...
  "sf", "ggplot2", "patchwork", "pROC", "kableExtra", "mgcv", "kknn"
)
```
...
Logistic Ridge penalises the sum of squared coefficient values ($\ell_2$ penalty), which shrinks all coefficients toward zero but never sets them exactly to zero — unlike LASSO there is no implicit variable selection. We use `glmnet` with `family = "binomial"` and `alpha = 0` (pure Ridge). The same model matrix (`X_train`, `X_test`) and fold assignments (`cv_folds`) from the LASSO section are reused for a fair comparison. $\lambda$ is selected by maximising cross-validated AUC.

```{r load-glmnet-ridge, include=FALSE}
if (!requireNamespace("glmnet", quietly = TRUE)) install.packages("glmnet")
library(glmnet)
```

```{r ridge-matrix, include=FALSE}
...
```{r ridge-cv-plot, fig.width=7, fig.height=4}
cv_df_ridge <- tibble(
...
ggplot(cv_df_ridge, aes(x = log_lambda, y = cvm)) +
  geom_ribbon(aes(ymin = cvlo, ymax = cvup), alpha = 0.15, fill = "#1a9641") +
  geom_line(colour = "#1a9641") +
  geom_point(size = 0.8, colour = "#1a9641") +
  geom_vline(xintercept = log(cv_ridge$lambda.min), linetype = "dashed",
             colour = "#d73027", linewidth = 0.7) +
  geom_vline(xintercept = log(cv_ridge$lambda.1se), linetype = "dashed",
             colour = "#fdae61", linewidth = 0.7) +
  annotate("text", x = log(cv_ridge$lambda.min), y = min(cv_df_ridge$cvlo),
           label = "λ.min", colour = "#d73027", hjust = -0.1, size = 3.5) +
  annotate("text", x = log(cv_ridge$lambda.1se), y = min(cv_df_ridge$cvlo),
           label = "λ.1se", colour = "#b36200", hjust = -0.1, size = 3.5) +
  labs(
    x     = "log(λ)",
    y     = "10-fold CV AUC",
    title = "Ridge — Cross-validated AUC vs regularisation strength"
  ) +
  theme_minimal(base_size = 11) +
  theme(panel.grid.minor = element_blank())
```

## Top Coefficients at λ.min

Unlike LASSO, Ridge retains all predictors. We display the top 30 by absolute coefficient magnitude.

```{r ridge-coef-plot, fig.width=8, fig.height=6}
cf_ridge <- coef(cv_ridge, s = "lambda.min")
coef_ridge_df <- tibble(
  feature  = rownames(cf_ridge)[-1],
  estimate = as.numeric(cf_ridge[-1])
) %>%
...
ggplot(coef_ridge_df, aes(x = estimate, y = reorder(feature, estimate), fill = direction)) +
  geom_col(width = 0.7) +
  geom_vline(xintercept = 0, linewidth = 0.4) +
  scale_fill_manual(values = c("Increases odds" = "#d73027", "Decreases odds" = "#2c7bb6")) +
  labs(
    x     = "Ridge coefficient (log-odds scale, λ.min)",
    y     = NULL,
    fill  = NULL,
    title = "Ridge — Top 30 coefficients by absolute magnitude"
  ) +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom", panel.grid.minor = element_blank())
```
...
```{r ridge-eval}
prob_ridge <- as.numeric(predict(cv_ridge, newx = X_test, s = "lambda.min", type = "response"))
roc_ridge  <- pROC::roc(y_test, prob_ridge, quiet = TRUE)
auc_ridge  <- as.numeric(pROC::auc(roc_ridge))
thr_ridge  <- best_thresh(roc_ridge)
class_ridge <- as.integer(prob_ridge >= thr_ridge)
met_ridge   <- class_metrics(y_test, class_ridge)

cat(sprintf(
  "AUC: %.4f  |  Threshold (Youden): %.4f\nSensitivity: %.3f  |  Specificity: %.3f  |  Balanced accuracy: %.3f\n",
  auc_ridge, thr_ridge, met_ridge$sensitivity, met_ridge$specificity, met_ridge$balanced_acc
))
```

```{r ridge-cm-plot, fig.width=4, fig.height=3.5}
plot_cm(y_test, class_ridge, title = "Ridge — Confusion Matrix (Youden threshold)")
```

```{r ridge-metrics-table}
compute_metrics(y_test, prob_ridge, "Ridge (λ.min)") %>%
  kbl(
    caption   = "Ridge — full evaluation metrics (test set, Youden threshold)",
    col.names = c("Model","AUC","PR-AUC","Log-Loss","Brier",
                  "Threshold","Sensitivity","Specificity","Precision","F1","MCC","Bal. Acc.")
  ) %>%
  kable_styling(bootstrap_options = c("striped","condensed"), full_width = TRUE)
...
```{r ridge-eval-plots, fig.width=10, fig.height=8}
eval_quad_plot(y_test, prob_ridge, "Ridge (λ.min)")
```
...
K-Nearest Neighbours classifies each observation by a kernel-weighted vote among its $k$ nearest training neighbours. KNN is non-parametric and distance-based, requiring consistent feature scales. Categorical predictors enter via Gower's distance (via `kknn`), which handles mixed-type inputs without manual dummy encoding; `scale = TRUE` standardises numeric columns internally.

Two choices shape the training data:

-   **Class balance**: equal draws from arrested = 0 and arrested = 1 give KNN balanced local neighbourhoods; without this the dominant class drowns minority-class signal.
-   **Size cap (15 000 rows)**: LOO-CV for $k$ selection scales $O(n^2)$ — 15 000 rows makes it tractable.

The test set is evaluated at its original size and class distribution.

```{r load-kknn, include=FALSE}
if (!requireNamespace("kknn", quietly = TRUE)) install.packages("kknn")
```

## Balanced Downsample and Scaling

We dummy-encode `f_final` via `model.matrix` (same approach as Ridge/LASSO), then scale using training-set statistics so no test information leaks into the scaling step.

```{r knn-downsample}
set.seed(2026)
idx_pos  <- which(train$arrested == 1)
idx_neg  <- which(train$arrested == 0)
n_per_cl <- min(floor(15000 / 2), length(idx_pos))

# Raw balanced subsample (factor columns still needed for model.matrix)
train_knn_raw <- bind_rows(
  train[sample(idx_pos, n_per_cl), ],
  train[sample(idx_neg, n_per_cl), ]
)

cat(sprintf(
  "KNN training set: %d rows  |  Arrested: %.1f%%  (n_per_class = %d)\n",
  nrow(train_knn_raw), 100 * mean(train_knn_raw$arrested), n_per_cl
))
```

```{r knn-scale}
# Dummy-encode (same columns as ridge/lasso X matrices)
X_knn_tr_raw <- model.matrix(f_final, data = train_knn_raw)[, -1]
X_knn_te_raw <- model.matrix(f_final, data = test)[, -1]

# Scale with train statistics; guard against zero-variance and NA sds
tr_means <- colMeans(X_knn_tr_raw, na.rm = TRUE)
tr_sds   <- apply(X_knn_tr_raw, 2, sd, na.rm = TRUE)
tr_sds[is.na(tr_sds) | tr_sds == 0] <- 1  # constant columns get sd=0 or NA

X_knn_tr <- scale(X_knn_tr_raw, center = tr_means, scale = tr_sds)
X_knn_te <- scale(X_knn_te_raw, center = tr_means, scale = tr_sds)

# Replace any residual NaN (e.g. new factor levels in test not seen in train)
X_knn_tr[!is.finite(X_knn_tr)] <- 0
X_knn_te[!is.finite(X_knn_te)] <- 0

# Sanitise column names (model.matrix can produce names with spaces/brackets)
colnames(X_knn_tr) <- make.names(colnames(X_knn_tr), unique = TRUE)
colnames(X_knn_te) <- make.names(colnames(X_knn_te), unique = TRUE)

# Attach response for kknn formula interface; must be a factor for classification mode
# (integer response triggers regression mode and suppresses the $prob matrix)
resp_levels <- c("0", "1")
df_knn_tr <- as.data.frame(X_knn_tr)
df_knn_tr$arrested <- factor(train_knn_raw$arrested, levels = resp_levels)
df_knn_te <- as.data.frame(X_knn_te)
df_knn_te$arrested <- factor(test$arrested, levels = resp_levels)

f_knn <- arrested ~ .

cat("KNN feature matrix:", nrow(X_knn_tr), "×", ncol(X_knn_tr), "\n")
```
...
set.seed(2026)
knn_tune <- kknn::train.kknn(
  f_knn, data  = df_knn_tr,
  kmax   = 21,
  kernel = "optimal"
)

best_k <- knn_tune$best.parameters$k
cat("Best k:", best_k, "\n")
```

```{r knn-tune-plot, fig.width=7, fig.height=3.5}
tibble(
  k     = seq_len(21),
  error = as.numeric(knn_tune$MISCLASS[, "optimal"])
) %>%
  ggplot(aes(x = k, y = error)) +
  geom_line(colour = "#2c7bb6") +
  geom_point(colour = "#2c7bb6", size = 2) +
  geom_vline(xintercept = best_k, linetype = "dashed", colour = "#d73027") +
  annotate("text", x = best_k, y = max(as.numeric(knn_tune$MISCLASS[, "optimal"])),
           label = paste0("k = ", best_k), colour = "#d73027",
           hjust = -0.15, size = 3.5) +
  labs(
    x     = "k (number of neighbours)",
    y     = "LOO misclassification rate",
    title = "KNN — LOO-CV misclassification rate vs k (optimal kernel)"
  ) +
  theme_minimal(base_size = 11) +
  theme(panel.grid.minor = element_blank())
```
...
# Positive class column: kknn coerces response to factor; locate "1" level
pos_col  <- which(colnames(knn_fit$prob) == "1")
if (length(pos_col) == 0) pos_col <- ncol(knn_fit$prob)
prob_knn  <- as.numeric(knn_fit$prob[, pos_col])
roc_knn   <- pROC::roc(test$arrested, prob_knn, quiet = TRUE)
auc_knn   <- as.numeric(pROC::auc(roc_knn))
thr_knn   <- best_thresh(roc_knn)
class_knn <- as.integer(prob_knn >= thr_knn)
met_knn   <- class_metrics(test$arrested, class_knn)

cat(sprintf(
  "AUC: %.4f  |  Threshold (Youden): %.4f\nSensitivity: %.3f  |  Specificity: %.3f  |  Balanced accuracy: %.3f\n",
  auc_knn, thr_knn, met_knn$sensitivity, met_knn$specificity, met_knn$balanced_acc
))
```

```{r knn-cm-plot, fig.width=4, fig.height=3.5}
plot_cm(test$arrested, class_knn,
        title = sprintf("KNN (k = %d) — Confusion Matrix (Youden threshold)", best_k))
```

```{r knn-metrics-table}
compute_metrics(test$arrested, prob_knn, sprintf("KNN (k = %d)", best_k)) %>%
  kbl(
    caption   = sprintf("KNN (k = %d) — full evaluation metrics (test set, Youden threshold)", best_k),
    col.names = c("Model","AUC","PR-AUC","Log-Loss","Brier",
                  "Threshold","Sensitivity","Specificity","Precision","F1","MCC","Bal. Acc.")
  ) %>%
  kable_styling(bootstrap_options = c("striped","condensed"), full_width = TRUE)
...
```{r knn-eval-plots, fig.width=10, fig.height=8}
eval_quad_plot(test$arrested, prob_knn, sprintf("KNN (k = %d)", best_k))
```
...
bind_rows(
  ...
  kable_styling(bootstrap_options = c("striped","hover","condensed"), full_width = TRUE) %>%
  row_spec(
    which.max(c(as.numeric(pROC::auc(roc_logit)), auc_lda, auc_qda, auc_nb, auc_ridge, auc_lasso_min, auc_lasso_1se, auc_knn, auc_gam)),
...
## QDA — Posterior Probability Separation

QDA does not produce a single linear score; it estimates class-conditional Gaussians with separate covariance matrices per class and returns posterior probabilities directly. Plotting the distribution of $\hat{P}(\text{arrested} = 1 \mid \mathbf{x})$ by true class reveals whether QDA assigns high confidence to the correct class: a well-calibrated and discriminating model concentrates the positive class near 1 and the negative class near 0.

```{r qda-posterior-density, fig.width=8, fig.height=4, message=FALSE, warning=FALSE}
qda_post <- data.frame(
  prob_arrested = predict(mod_qda, newdata = test)$posterior[, "1"],
  arrested      = factor(test$arrested, levels = c(0, 1),
                         labels = c("Not arrested (0)", "Arrested (1)"))
)

ggplot(qda_post, aes(x = prob_arrested, fill = arrested, colour = arrested)) +
  geom_density(alpha = 0.35, linewidth = 0.8) +
  scale_fill_manual(values   = c("Not arrested (0)" = "#4575b4",
                                  "Arrested (1)"     = "#d73027")) +
  scale_colour_manual(values = c("Not arrested (0)" = "#4575b4",
  ...
  labs(
    x        = expression(hat(P)(arrested == 1 ~ "|" ~ bold(x))),
    y        = "Density",
    fill     = NULL, colour = NULL,
    title    = "QDA — Distribution of posterior P(arrested=1) by true class (test set)",
    subtitle = "Ideal: negatives pile near 0, positives near 1"
  ) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top", panel.grid.minor = element_blank())
```

If the two density curves overlap substantially in the middle of the probability axis, QDA is uncertain for many observations—reflecting either genuine noise in the features or a violation of its Gaussian equal-covariance assumption (note: unlike LDA, QDA allows unequal covariances, which gives it more flexibility but also more variance in small samples or with sparse classes).

---

## Ridge — Coefficient Paths and CV Deviance vs $\lambda$

The coefficient path shows how each predictor's coefficient shrinks as the $\ell_2$ penalty $\lambda$ increases. Unlike LASSO, Ridge never zeroes out coefficients — all shrink smoothly toward zero. The CV deviance curve (right panel) identifies $\lambda_{\min}$ (lowest CV loss) and $\lambda_{\text{1se}}$ (most regularised model within one standard error).

```{r ridge-diag, fig.width=12, fig.height=5, message=FALSE, warning=FALSE}
# Coefficient path
ridge_fit   <- cv_ridge$glmnet.fit
coef_mat_r  <- as.matrix(coef(ridge_fit))[-1, ]   # drop intercept row
log_lam_r   <- log(ridge_fit$lambda)

coef_path_r <- as.data.frame(t(coef_mat_r)) %>%
  mutate(log_lambda = log_lam_r) %>%
  tidyr::pivot_longer(-log_lambda, names_to = "predictor", values_to = "coef")

p_ridge_path <- ggplot(coef_path_r,
                        aes(x = log_lambda, y = coef, group = predictor)) +
  geom_line(alpha = 0.35, linewidth = 0.5, colour = "#4575b4") +
  geom_vline(xintercept = log(cv_ridge$lambda.min), linetype = "dashed",
             colour = "firebrick", linewidth = 0.7) +
  geom_vline(xintercept = log(cv_ridge$lambda.1se), linetype = "dotted",
...
p_ridge_cv <- ggplot(cv_df_r, aes(x = log_lambda, y = cvm)) +
  geom_ribbon(aes(ymin = cvlo, ymax = cvup), alpha = 0.15, fill = "#4575b4") +
  ...
  labs(x = expression(log(lambda)), y = "CV Binomial Deviance",
       title = "Ridge — Cross-validated deviance",
       subtitle = "Ribbon = ±1 SE") +
  theme_minimal(base_size = 11) +
  theme(panel.grid.minor = element_blank())

p_ridge_path + p_ridge_cv
```

Ridge paths confirm that no predictor dominates: all coefficients shrink proportionally, and the flat plateau at large $\lambda$ is expected — Ridge always retains the full predictor set. The CV curve's minimum locates the optimal bias–variance trade-off; if $\lambda_{\min}$ and $\lambda_{\text{1se}}$ are far apart, the model is sensitive to regularisation strength.
