...
-   The small window of time used to gather the data (around 3ys);
...
However, there is a caveat: the data spans throughout 3 years. This is one of the constraints that we cannot remove, but the volume of the data will still help us back up our claims in the end.
...
```{r count-veh-checkpoints-ethnicity-unknown}
vehiclestops %>%
  filter(
   ethnicity == "UNKNOWN" &
   veh_checkpoint == "true"
...
```{r count-veh-checkpoints-rare-ethnicities}
vehiclestops %>%
  filter(
   ethnicity %in% c("AMERICAN INDIAN/ALASKAN NATIVE", "UNKNOWN", "OTHER") &
   veh_checkpoint == "true"
...
# Select the relevant columns (must be 0/1 or logical)
stops_bin <- vehiclestops %>%
  dplyr::select(age_unknown, ethnicity_unknown, sex_unknown, veh_checkpoint)
...
upset(
  stops_bin, 
  sets = c("age_unknown","ethnicity_unknown","sex_unknown", "veh_checkpoint"),
  sets.bar.color = "firebrick", 
  order.by = "freq"
)
```
