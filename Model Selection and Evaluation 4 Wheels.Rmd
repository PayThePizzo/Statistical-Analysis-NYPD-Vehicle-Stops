...
```{r knn-scale}
# 1. Align and clean missing values using model.frame
mm_knn_train <- model.frame(f_final, data = train_knn_raw, na.action = na.omit)
X_knn_tr_raw <- model.matrix(f_final, data = mm_knn_train)[, -1]
y_knn_train  <- mm_knn_train$arrested

mm_knn_test  <- model.frame(f_final, data = test, na.action = na.omit)
X_knn_te_raw <- model.matrix(f_final, data = mm_knn_test)[, -1]
y_knn_test   <- mm_knn_test$arrested

# 2. Scale with train statistics (Keep your existing scaling logic here)
...
# 3. Attach the correctly aligned response vectors
resp_levels <- c("0", "1")
df_knn_tr <- as.data.frame(X_knn_tr)
df_knn_tr$arrested <- factor(y_knn_train, levels = resp_levels) # <-- Fixed

df_knn_te <- as.data.frame(X_knn_te)
df_knn_te$arrested <- factor(y_knn_test, levels = resp_levels)   # <-- Fixed
...
# CHANGE test$arrested TO y_knn_test TO MATCH COV-MATRIX ROWS
roc_knn   <- pROC::roc(y_knn_test, prob_knn, quiet = TRUE) # <-- Fixed
auc_knn   <- as.numeric(pROC::auc(roc_knn))
thr_knn   <- best_thresh(roc_knn)
class_knn <- as.integer(prob_knn >= thr_knn)
met_knn   <- class_metrics(y_knn_test, class_knn)          # <-- Fixed
...
plot_cm(y_knn_test, class_knn,
        title = sprintf("KNN (k = %d) — Confusion Matrix (Youden threshold)", best_k))
```

```{r knn-metrics-table}
compute_metrics(y_knn_test, prob_knn, sprintf("KNN (k = %d)", best_k)) %>%
  kbl(
...
eval_quad_plot(y_knn_test, prob_knn, sprintf("KNN (k = %d)", best_k))
```
